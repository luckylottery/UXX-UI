...
export default {
  ...
  methods: {
    ...
    handleResize() {
      this.width = Math.min.apply(null, [window.outerWidth, window.innerWidth])
      this.boxShow = false
    },
    stakeSlideRight() {
    ...
    clickMiniChart(value, index) {
      this.boxShow = true
      this.chartValue = `Maturrity JAN-2026 Yield  JAN-2026`

      if (this.width >= 300 && this.width < 400) {
        this.boxLeftBottom = `left: ${-28 + index * 11}px; bottom: ${170 + index * 2.9}px;`
      } else if (this.width >= 400 && this.width < 504) {
        this.boxLeftBottom = `left: ${-20 + index * 14}px; bottom: ${170 + index * 3.2}px;`
      } else if (this.width >= 504 && this.width < 575) {
        this.boxLeftBottom = `left: ${-20 + index * 19}px; bottom: ${170 + index * 3.2}px;`
